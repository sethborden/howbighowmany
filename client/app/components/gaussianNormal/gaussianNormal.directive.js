...
.directive('gaussianNormal', function ($timeout) {
    ...
    link: function (scope, element, attrs) {
        ...
        var randomGaussianArray = function(count, max) {
            var data = [];
            for(var i = 0; i < count; i++) {
                //Because central limit theorem
                data.push((randomFloat(max) + randomFloat(max) + randomFloat(max) + randomFloat(max) + randomFloat(max) + randomFloat(max) -3) / 3);
            }
            return data;
        };

        //Utility functions to generate the histogram
        ...
        // Utility functions to generate the bell curve
        ...
        var stddev = function(data) {
            ...
            return Math.sqrt(sum(vSquared) / (data.length - 1)); // Bessel's Correction
        };

        var bellCurve = function(x, mean, sigma) {
